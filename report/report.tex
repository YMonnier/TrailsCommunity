...
\subsection{Exigence fonctionnelle 1.5}

\textbf{ID : FR5}

TITRE : Un utilisateur peut rejoindre une session.

DESCRIPTION : Étant donné qu'un utilisateur s'est connecté, il doit à le choix entre 3 types de sessions. Le premier type sont les sessions actives. Elle sont soit privé ou publique. Si elle sont privé, l'utilisateur doit connaitre le mot de passe pour les rejoindre. Le deuxième type est les sessions dont il a déjà participé et dont celle-ci sont clôturé. La dernière est les sessions que l'utilisateurs à créées.

BUT : Pour qu'un organisateur puisse rejoindre une session sur l'application mobile.

DÉPENDANCE : FR1, FR3, FR4.

\subsection{Exigence fonctionnelle 1.6}

\textbf{ID : FR6}

TITRE : Un utilisateur peut créer une session.

DESCRIPTION : Étant donné qu'un utilisateur s'est connecté, il doit pouvoir créer une nouvelle session dans l'application. Les informations de la nouvelle session créée seront stockées dans la base de données. L'utilisateur doit fournir en premier lieu le lieu de départ ainsi que le lieu d'arrivé. Le format d'une adresse lui sera demandé. L'application convertira cette adresse en coordonnées GPS. De plus lui seront demandé le type d'activité ainsi que la date de départ. Pour finir, un mot de passe l'organisateur doit choisir un mot de passe qu'il divulguera à l'ensemble des participants pour qu'ils puissent rejoindre la session.
...
\subsection{Exigence fonctionnelle 1.8}
...
\subsection{Exigence fonctionnelle 1.9}
